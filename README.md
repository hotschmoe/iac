...
**Implemented:** WebSocket networking (server accept/broadcast, client connect/poll). SQLite schema and full CRUD (players, fleets, ships, sectors, server state). Stochastic combat with rapid-fire chains and NPC encounters on sector entry. Movement with speed-based cooldowns and fuel consumption. Resource harvesting with density-based yield, cargo limits, and sector depletion (density downgrades as resources are extracted, thresholds persisted to DB). Harvest validation rejects invalid states (moving, no resources, cargo full) with error feedback to the client. Emergency recall with fuel cost and stochastic hull damage. Player reconnection across sessions. Dirty state tracking with batch persistence every 30 ticks. Graceful shutdown. Zithril TUI with command center, windshield, and star map views. Event log ring buffer displayed in both command center and windshield views (owns duped alert messages to avoid use-after-free from parse arena and silent mutation from shared status buffer). Server sends sector state (terrain, resources, connections, hostile fleet info) to clients on connect and per-tick. Windshield hex node graph with spatially-positioned direction slots radiating from center sector, connection lines, came-from indicator after movement, resource/exploration summaries per neighbor, and compact text fallback for small terminals. Fixed direction keys (1=E, 2=NE, 3=NW, 4=W, 5=SW, 6=SE). NPC stat scaling by zone distance (inner ring 0.6x, making early encounters winnable). Per-fleet hostile display showing ship class, count, and behavior. Resource regeneration (depleted sectors recover density over hours when unoccupied). NPC patrol AI (patrol/aggressive NPCs move between sectors, engage player fleets, respawn after zone-based delay). Hex map rendering with flat-top axial projection, zoom levels (close/sector/region), fog-of-war, and symbol classification (@=fleet, H=home, !=hostile, terrain for resources).
